...
public class ThunderTypeAdapter extends TypeAdapter<Thunder> {
    String path = System.getProperty("user.dir") + "/src/main/resources/Icons/";

    @Override
    public void write(JsonWriter out, Thunder w) throws IOException {
        out.beginObject();
        out.name("Class Name").value(Thunder.class.getName());
        ...
        out.name("Slayer").value(array.toString()); out.name("Dropable").value(w.isDropable());
        out.endObject();
    }

    @Override
    public Thunder read(JsonReader in) throws IOException {
        ...
        Thunder w = new Thunder(name, might, weight, level, hit, Use, crit, range_1, range_2, exp, physical, price, image, slayer,drop);
        w.setUses(use);
        return w;
    }
}

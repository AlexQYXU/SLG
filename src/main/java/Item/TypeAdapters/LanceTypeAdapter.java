...
public class LanceTypeAdapter extends TypeAdapter<Lance> {
    String path = System.getProperty("user.dir") + "/src/main/resources/Icons/";

    @Override
    public void write(JsonWriter out, Lance w) throws IOException {
        out.beginObject();
        out.name("Class Name").value(Lance.class.getName());
    ...
    @Override
    public Lance read(JsonReader in) throws IOException {
        ...
        Lance w = new Lance(name, might, weight, level, hit, Use, crit, range_1, range_2, exp, physical, price, image, slayer,drop);
        w.setUses(use);
        return w;
    }
}

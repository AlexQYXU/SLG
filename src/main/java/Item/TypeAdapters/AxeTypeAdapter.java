...
public class AxeTypeAdapter extends TypeAdapter<Axe> {
    String path = System.getProperty("user.dir") + "/src/main/resources/Icons/";

    @Override
    public void write(JsonWriter out, Axe w) throws IOException {
        out.beginObject();
        out.name("Class Name").value(Axe.class.getName());
    ...
    @Override
    public Axe read(JsonReader in) throws IOException {
        ...
        Axe w = new Axe(name, might, weight, level, hit, Use, crit, range_1, range_2, exp, physical, price, image, slayer);
        w.setUses(use);
        w.setDropable(drop);
        return w;
    }

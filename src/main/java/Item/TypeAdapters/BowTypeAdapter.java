...
public class BowTypeAdapter extends TypeAdapter<Bow> {
    String path = System.getProperty("user.dir") + "/src/main/resources/Icons/";

    @Override
    public void write(JsonWriter out, Bow w) throws IOException {
        out.beginObject();
        out.name("Class Name").value(Bow.class.getName());
    ...
    @Override
    public Bow read(JsonReader in) throws IOException {
        ...
        Bow w = new Bow(name, might, weight, level, hit, Use, crit, range_1, range_2, exp, physical, price, image, slayer,drop);
        w.setUses(use);
        return w;
    }
}

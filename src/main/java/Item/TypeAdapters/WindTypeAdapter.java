...
public class WindTypeAdapter extends TypeAdapter<Wind> {
    String path = System.getProperty("user.dir") + "/src/main/resources/Icons/";

    @Override
    public void write(JsonWriter out, Wind w) throws IOException {
        out.beginObject();
        out.name("Class Name").value(Wind.class.getName());
    ...
    @Override
    public Wind read(JsonReader in) throws IOException {
        ...
        Wind w = new Wind(name, might, weight, level, hit, Use, crit, range_1, range_2, exp, physical, price, image, slayer, drop);
        w.setUses(use);
        return w;
    }
}

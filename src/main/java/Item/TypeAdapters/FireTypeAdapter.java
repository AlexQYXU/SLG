...
public class FireTypeAdapter extends TypeAdapter<Fire> {
    String path = System.getProperty("user.dir") + "/src/main/resources/Icons/";

    @Override
    public void write(JsonWriter out, Fire w) throws IOException {
        out.beginObject();
        out.name("Class Name").value(Fire.class.getName());
    ...
    @Override
    public Fire read(JsonReader in) throws IOException {
        ...
        Fire w = new Fire(name, might, weight, level, hit, Use, crit, range_1, range_2, exp, physical, price, image, slayer,drop);
        w.setUses(use);
        return w;
    }
}
